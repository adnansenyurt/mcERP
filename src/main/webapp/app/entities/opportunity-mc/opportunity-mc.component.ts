import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpResponse, HttpErrorResponse } from '@angular/common/http';
import { Subscription } from 'rxjs/Subscription';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { OpportunityMc } from './opportunity-mc.model';
import { OpportunityMcService } from './opportunity-mc.service';
import { Principal } from '../../shared';

@Component({
    selector: 'jhi-opportunity-mc',
    templateUrl: './opportunity-mc.component.html'
})
export class OpportunityMcComponent implements OnInit, OnDestroy {
opportunities: OpportunityMc[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private opportunityService: OpportunityMcService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {
    }

    loadAll() {
        this.opportunityService.query().subscribe(
            (res: HttpResponse<OpportunityMc[]>) => {
                this.opportunities = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }
    ngOnInit() {
        this.loadAll();
        this.principal.identity().then((account) => {
            this.currentAccount = account;
        });
        this.registerChangeInOpportunities();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: OpportunityMc) {
        return item.id;
    }
    registerChangeInOpportunities() {
        this.eventSubscriber = this.eventManager.subscribe('opportunityListModification', (response) => this.loadAll());
    }

    private onError(error) {
        this.jhiAlertService.error(error.message, null, null);
    }
}
